    ...
    class Program
    {
        static void Main(string[] args)
        {
            var result = CommandLine.Parser.Default.ParseArguments
                                    <QSolveOption, QModelOption, QStatOption, SolveOption, AnalyzeOption, ConvStatOption, MergeOption, BuildOption, TestOption>(args);
            result.WithParsed<QSolveOption>(opt => Command.QSolve(opt.ModelName))
                  .WithParsed<QModelOption>(opt => Command.QModel(opt.ModelName))
                  .WithParsed<QStatOption>(opt => Command.QStatistic(opt.FilePath))
                  .WithParsed<SolveOption>(opt => Command.Solve(opt.InputFile, opt.OutputFile, opt.ModelName))
                  .WithParsed<AnalyzeOption>(opt => Command.Analyze(opt))
                  .WithParsed<ConvStatOption>(opt => Command.ConvStat(opt))
                  .WithParsed<MergeOption>(opt => Command.Merge(opt.FilePaths, opt.OutputFile, opt.MinRate))
